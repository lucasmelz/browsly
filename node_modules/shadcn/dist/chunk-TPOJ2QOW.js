...
If this is a remote registry, you may need to authenticate or a token.`);let m=await l.json(),h=m&&typeof m=="object"&&"error"in m?m.error:l.statusText||f[l.status];throw new Error(`Failed to fetch from ${u.info(s)}.
${h}`)}return l.json()})();return t.useCache&&A.set(s,o),o}))}catch(r){return d.error(`
`),b(r),[]}}function Ir(){A.clear();}async function et(e,t){try{if(!await Ze())return null;e.includes("index")&&e.unshift("index");let i=await it(e,t),s=await P(i),o=z.array(k).parse(s);if(!o)return null;if(e.includes("index")&&t.tailwind.baseColor){let c=await rt(t.tailwind.baseColor,t);c&&o.unshift(c);}o.sort((c,g)=>c.type==="registry:theme"?-1:1);let l={};o.forEach(c=>{l=T(l,c.tailwind??{});});let f={};o.forEach(c=>{f=T(f,c.cssVars??{});});let m={};o.forEach(c=>{m=T(m,c.css??{});});let h="";return o.forEach(c=>{c.docs&&(h+=`${c.docs}
`);}),J.parse({dependencies:T.all(o.map(c=>c.dependencies??[])),devDependencies:T.all(o.map(c=>c.devDependencies??[])),files:T.all(o.map(c=>c.files??[])),tailwind:l,cssVars:f,css:m,docs:h})}catch(r){return b(r),null}}async function tt(e,t){let r=new Set,i=[],s=t.resolvedPaths?.cwd?await q(t.resolvedPaths.cwd,t.style):t.style;async function o(l){let f=oe(W(l)?l:`styles/${s}/${l}.json`);if(!r.has(f)){r.add(f);try{let[m]=await P([f]),h=k.parse(m);if(i.push(f),h.registryDependencies)for(let c of h.registryDependencies)await o(c);}catch(m){console.error(`Error fetching or parsing registry item at ${l}:`,m);}}}return await o(e),Array.from(new Set(i))}async function rt(e,t){let[r,i]=await Promise.all([qe(e),H(t)]);if(!r)return null;let s={name:e,type:"registry:theme",tailwind:{config:{theme:{extend:{borderRadius:{lg:"var(--radius)",md:"calc(var(--radius) - 2px)",sm:"calc(var(--radius) - 4px)"},colors:{}}}}},cssVars:{theme:{},light:{radius:"0.5rem"},dark:{}}};return t.tailwind.cssVariables&&(s.tailwind.config.theme.extend.colors={...s.tailwind.config.theme.extend.colors,...ne(r.cssVars.dark??{})},s.cssVars={theme:{...r.cssVars.theme,...s.cssVars.theme},light:{...r.cssVars.light,...s.cssVars.light},dark:{...r.cssVars.dark,...s.cssVars.dark}},i==="v4"&&r.cssVarsV4&&(s.cssVars={theme:{...r.cssVarsV4.theme,...s.cssVars.theme},light:{radius:"0.625rem",...r.cssVarsV4.light},dark:{...r.cssVarsV4.dark}})),s}function oe(e){if(W(e)){let t=new URL(e);return t.pathname.match(/\/chat\/b\//)&&!t.pathname.endsWith("/json")&&(t.pathname=`${t.pathname}/json`),t.toString()}return `${Ye}/${e}`}function W(e){try{return new URL(e),!0}catch{return !1}}async function it(e,t){let r=[];for(let i of e){let s=await tt(i,t);r.push(...s);}return Array.from(new Set(r))}function Ar(){return new Map([["registry:ui","ui"],["registry:lib","lib"],["registry:hook","hooks"],["registry:block","components"],["registry:component","components"]])}function Rr(e){let t=new Map;return e.forEach(r=>{r.registryDependencies&&r.registryDependencies.forEach(i=>{t.set(i,r);});}),t}

export { ce as A, k as B, nt as C, V as D, M as E, B as F, G, J as H, Qe as I, Ze as J, Tr as K, vr as L, Er as M, Sr as N, qe as O, Xe as P, jr as Q, Lr as R, P as S, Ir as T, et as U, rt as V, W, it as X, Ar as Y, Rr as Z, u as a, x as b, Ft as c, zt as d, Dt as e, _t as f, Q as g, E as h, F as i, Ut as j, je as k, Ie as l, O as m, L as n, Ct as o, H as p, d as q, X as r, b as s, cr as t, te as u, We as v, N as w, ae as x, le as y, R as z };
//# sourceMappingURL=out.js.map
//# sourceMappingURL=chunk-TPOJ2QOW.js.map